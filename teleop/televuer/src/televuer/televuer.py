...
class TeleVuer:
    def __init__(self, use_hand_tracking: bool, binocular: bool=True, img_shape: tuple=None, display_fps: float=30.0,
        ...
        xr_mode = "immersive-ar" if display_mode == "pass-through" else "immersive-vr"
        self.vuer = Vuer(host='0.0.0.0', cert=cert_file, key=key_file, queries=dict(grid=False, xrMode=xr_mode), queue_len=3)
        ...
        self.wrist_webrtc_urls = wrist_webrtc_urls or {}

        # Debug: red square image (head-following HUD) for VR rendering verification
        if self.debug:
            self._debug_img = np.zeros((64, 64, 3), dtype=np.uint8)
            self._debug_img[:, :, 0] = 255  # red in RGB

        # Wrist camera shared memory for ZMQ mode
        if self.wrist_view and not self.webrtc:
            wrist_shape = (480, 640, 3)  # standard wrist camera resolution
            self.left_wrist_shm = shared_memory.SharedMemory(create=True, size=np.prod(wrist_shape) * np.uint8().itemsize)
            self.left_wrist_img = np.ndarray(wrist_shape, dtype=np.uint8, buffer=self.left_wrist_shm.buf)
            self.right_wrist_shm = shared_memory.SharedMemory(create=True, size=np.prod(wrist_shape) * np.uint8().itemsize)
            self.right_wrist_img = np.ndarray(wrist_shape, dtype=np.uint8, buffer=self.right_wrist_shm.buf)
        ...
        print(f"[TeleVuer] display_mode={self.display_mode}, webrtc={self.webrtc}, zmq={self.zmq}, "
              f"binocular={self.binocular}, wrist_view={self.wrist_view}, fn={fn.__name__}")
        if self.webrtc:
            print(f"[TeleVuer] webrtc_url={self.webrtc_url}")
            if self.wrist_view and self.wrist_webrtc_urls:
                print(f"[TeleVuer] wrist_webrtc_urls={self.wrist_webrtc_urls}")
    ...
    def render_to_xr(self, image):
        if self.webrtc or self.display_mode == "pass-through":
            ...
            return
        bgr = self._extract_bgr(image)
        if bgr is None:
            return
        self.latest_frame = bgr
        self.new_frame_event.set()

    def _extract_bgr(self, image):
        """Extract BGR numpy array from TeleImage or raw numpy array.
        Tries .bgr first, falls back to decoding .jpg if .bgr is None."""
        if image is None:
            return None
        if not hasattr(image, 'bgr'):
            return image  # already a numpy array
        bgr = image.bgr
        if bgr is not None:
            return bgr
        # .bgr is None (decoder lag) - fallback to decoding .jpg synchronously
        if hasattr(image, 'jpg') and image.jpg is not None:
            bgr = cv2.imdecode(np.frombuffer(image.jpg, np.uint8), cv2.IMREAD_COLOR)
            return bgr
        return None

    def render_wrist_to_xr(self, left_image, right_image):
        if not self.wrist_view or self.webrtc or self.display_mode == "pass-through":
            return
        if left_image is not None:
            bgr = self._extract_bgr(left_image)
            if bgr is not None:
                rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
                self.left_wrist_img[:] = cv2.resize(rgb, (640, 480)) if rgb.shape[:2] != (480, 640) else rgb
        if right_image is not None:
            bgr = self._extract_bgr(right_image)
            if bgr is not None:
                rgb = cv2.cvtColor(bgr, cv2.COLOR_BGR2RGB)
                self.right_wrist_img[:] = cv2.resize(rgb, (640, 480)) if rgb.shape[:2] != (480, 640) else rgb
    ...
    def close(self):
        ...
        if self.wrist_view and not self.webrtc:
            try:
                self.left_wrist_shm.close()
                self.left_wrist_shm.unlink()
                self.right_wrist_shm.close()
                self.right_wrist_shm.unlink()
            except:
                pass
    ...
    async def main_image_binocular_zmq(self, session):
        ...
        while True:
            ...
            if self.wrist_view:
                elements.extend(self._get_wrist_zmq_elements())
            if self.debug:
                elements.extend(self._get_debug_elements())
    ...
    def _get_debug_elements(self):
        """Generate a red square HUD element for VR rendering verification."""
        return [
            ImageBackground(
                self._debug_img,
                format="png",
                key="debug-red-square",
                aspect=1.0,
                distanceToCamera=2,
                height=1.0,
            ),
    ...
    def _get_wrist_webrtc_elements(self):
        """Generate WebRTC wrist camera panel elements for VR display.

        position is relative to the camera (HUDPlane behavior):
          x: left(-) / right(+), y: down(-) / up(+), z: added to -distanceToCamera
        """
        elements = []
        left_url = self.wrist_webrtc_urls.get('left')
        right_url = self.wrist_webrtc_urls.get('right')
        if left_url:
            elements.append(
                WebRTCVideoPlane(
                    src=left_url,
    ...
    def _get_wrist_zmq_elements(self):
        """Generate ZMQ wrist camera panel elements for VR display.

        Uses ImageBackground (camera-facing HUD) so panels follow the user's head.
        distanceToCamera must be < head camera's distanceToCamera (1.0) to appear in front.
        """
        return [
            ImageBackground(
                self.left_wrist_img,
    ...
    async def main_image_binocular_webrtc(self, session):
        ...
        while True:
            ...
            if self.wrist_view:
                elements.extend(self._get_wrist_webrtc_elements())
            if self.debug:
                elements.extend(self._get_debug_elements())
            session.upsert(elements, to="bgChildren")
            await asyncio.sleep(1.0 / self.display_fps)
    ...
    async def main_pass_through(self, session):
        ...
        while True:
            if self.debug:
                session.upsert(self._get_debug_elements(), to="bgChildren")
            await asyncio.sleep(1.0 / self.display_fps)
