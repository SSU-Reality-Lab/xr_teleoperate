...
class BGRArrayVideoStreamTrack(MediaStreamTrack):
    ...
    async def recv(self) -> av.VideoFrame:
        ...
        if self._frame_count % 300 == 0:
            logger_mp.info(f"[WebRTC Track] sent {self._frame_count} frames, dropped {self._drop_count}")
        return frame
...
class WebRTC_PublisherThread(threading.Thread):
    ...
    async def _offer(self, request: web.Request) -> web.Response:
        ...
        logger_mp.info(f"[WebRTC] /offer request from {client_ip} on port {self._port}")
        params = await request.json()
        offer = RTCSessionDescription(sdp=params["sdp"], type=params["type"])
        logger_mp.debug(f"[WebRTC] SDP offer type={params['type']}, sdp length={len(params['sdp'])}")
        ...
        async def on_connectionstatechange():
            logger_mp.info(f"[WebRTC] Connection state: {pc.connectionState} (port {self._port}, client {client_ip})")
            if pc.connectionState in ["failed", "closed"]:
                await self._cleanup_pc(pc)

        @pc.on("iceconnectionstatechange")
        async def on_iceconnectionstatechange():
            logger_mp.info(f"[WebRTC] ICE state: {pc.iceConnectionState} (port {self._port}, client {client_ip})")
